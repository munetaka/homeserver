...
- 電気料金メトリクス: `cost_day_yen{kind=buy|savings|sell_income}`(日次、限界単価ベース)と
  ...
  **cost_* 系列は全て導出データ**なので、料金モデルを修正したら
  `delete_series match[]={__name__=~"cost_(day|period|year)_yen"}` で消して
  `el cost-update --since 2025-06-01` で全再計算するのが正しい修正手順
  (日次の上書きは VM の `-dedup.minScrapeInterval=1ms` で単一サンプルに収束するが、
  値が下がる方向の修正は max 優先の dedup に負けるため、消して再計算が確実)。
